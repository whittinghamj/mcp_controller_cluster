    ...
    for i in range(blinkt.NUM_PIXELS):
        ...
        if i in pixels:
            if cpu < 10:
                ...
                blinkt.set_pixel(i, 113,255,0)
            elif (cpu > 31) and (cpu < 40):
                blinkt.set_pixel(i, 170,255,0)
            elif (cpu > 41) and (cpu < 50): # Yellow
                blinkt.set_pixel(i, 226,255,0)
            elif (cpu > 51) and (cpu < 60):
                blinkt.set_pixel(i, 255,226,0)
            elif (cpu > 61) and (cpu < 70): # Orange
                blinkt.set_pixel(i, 255,170,0)
            elif (cpu > 71) and (cpu < 80):
                blinkt.set_pixel(i, 255,113,0)
            elif (cpu > 81) and (cpu < 90):
                blinkt.set_pixel(i, 255,56,0)
            else:
